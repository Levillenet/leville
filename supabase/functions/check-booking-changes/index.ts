...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const ticket of changeoverTickets) {
      try {
        // Fetch current booking situation from Beds24
        ...
        if (departureChanged && newDeparture) {
          // Cancel old scheduled changeover reminders
          await supabase
          ...
          // Schedule new reminder for the new departure morning
          const { email } = await resolveRecipientEmail(supabase, ticket.apartment_id, ticket.assignment_type || "kiinteistohuolto");
          if (email) {
            const reminderTime = new Date(newDeparture + "T05:50:00Z"); // morning of departure
            ...
            await supabase.from("ticket_history").insert({
              ...
              changed_by: "system",
              action_type: "updated",
              new_value: `Muistutus siirretty uuteen lähtöpäivään: ${newDepStr}`,
            });
          }
        }
